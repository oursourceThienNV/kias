...
export default function AnnouncementBar({
  ...
  const bg = current.backgroundColor || "#79192A";
    ...
    >
      <div className="flex items-center justify-center min-h-[38px] text-center px-4">
        {announcements.map((a, i) => (
          <div
            key={a.id}
            className={`absolute inset-0 flex items-center justify-center px-2 transition-opacity duration-500 ease-in-out ${
        ...
        <span className="invisible text-[13px] select-none">
          {current.message}
        </span>
      </div>
    </div>
